#!/usr/bin/env python3
"""
RPS UDP to ROS2 桥接节点

监听两个 UDP 端口：
1. 5005: AI 手势命令 (rock/paper/scissors) -> /rps/ai_move
2. 5007: 手臂控制命令 (raise/lower) -> /rps/arm_action
"""
import socket
...
class UdpToRosBridge(Node):
    def __init__(self):
        super().__init__("rps_udp_to_ros_bridge")

        # AI 手势命令发布者
        self.pub_move = self.create_publisher(String, "/rps/ai_move", 10)
        # 手臂控制命令发布者
        self.pub_arm = self.create_publisher(String, "/rps/arm_action", 10)
        ...
        self.get_logger().info(f"UDP listen {UDP_IP}:{UDP_PORT_MOVE} -> ROS2 /rps/ai_move")
        self.get_logger().info(f"UDP listen {UDP_IP}:{UDP_PORT_ARM} -> ROS2 /rps/arm_action")
    ...
    def _poll_move_socket(self):
        ...
        self.get_logger().info(f"Published /rps/ai_move: {cmd} (from UDP {addr})")
    ...
    def _poll_arm_socket(self):
        ...
        self.get_logger().info(f"Published /rps/arm_action: {cmd} (from UDP {addr})")
