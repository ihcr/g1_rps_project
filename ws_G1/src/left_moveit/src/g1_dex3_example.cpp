...
private:
    ...
    void rotateMotors(bool isLeftHand)
    {
        ...
        {
            RCLCPP_ERROR(this->get_logger(), "Invalid MOTOR_MAX value: %d", MOTOR_MAX);
            return;
        }
        ...
        for (int i = 0; i < MOTOR_MAX; i++)
        {
            ...
            if (i == 5 || i == 6)
            {
                ...
                if (debug_count++ % 1000 == 0)  // 每1000次循环打印一次
                {
                    RCLCPP_INFO(this->get_logger(), "Motor[%d] target q = %.3f (range: %.3f to %.3f)",
                                i, q, minLimits[i], maxLimits[i]);
                }
            }
        }
    ...
    void printState(bool isLeftHand)
    {
...

        {
            ...
            // 直接从 handState_ 读取，不复制整个消息
            for (int i = 0; i < 7; i++)
            {
                q(i) = handState_->motor_state[i].q;
        ...
        // 添加原始关节位置信息
        RCLCPP_INFO(this->get_logger(), "Raw joint positions (especially motor 5 & 6):");
        {
            std::lock_guard<std::mutex> lck(stateMtx_);
            for (int i = 0; i < 7; i++)
            {
                RCLCPP_INFO(this->get_logger(), "  Motor[%d]: q = %.3f", i, handState_->motor_state[i].q);
            }
        }
