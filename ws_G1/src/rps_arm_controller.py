...
# -*- coding: utf-8 -*-
"""
RPS 右臂控制节点 - 模拟石头剪刀布时的手臂摆动

订阅: /rps/arm_action (std_msgs/String) - "raise" 或 "lower"
发布: /rps/ai_move (std_msgs/String) - 抬起手臂时发送 "rock" 让夹爪握拳
通过: /right_arm_controller/follow_joint_trajectory Action 控制右臂

使用方法：
    python3 rps_arm_controller.py
...
class RPSArmController(Node):
    """RPS 右臂控制节点"""

    def __init__(self):
        super().__init__('rps_arm_controller')
        ...
        self.gripper_pub = self.create_publisher(String, '/rps/ai_move', 10)

        self.get_logger().info('RPS 手臂控制节点初始化中...')
    ...
    def _wait_for_server(self):
        ...
        if self.action_client.wait_for_server(timeout_sec=30.0):
            ...
            self.get_logger().info('等待 /rps/arm_action 命令...')
            self.get_logger().info('  - "raise": 抬起右臂30度')
            self.get_logger().info('  - "lower": 放回初始位置')
        else:
            self.get_logger().error('无法连接到 /right_arm_controller/follow_joint_trajectory')
    ...
    def raise_arm(self):
        ...
        thread = threading.Thread(
            ...
            args=(self.raise_pos, self.raise_duration, "抬起右臂", True),
            daemon=True
        )
        thread.start()
    ...
    def lower_arm(self):
        ...
        thread = threading.Thread(
            target=self._execute_trajectory,
            args=(self.init_pos, self.lower_duration, "放下右臂", False),
            daemon=True
        )
        thread.start()
